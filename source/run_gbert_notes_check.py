...
from predictive_models import GBERTNotesCheck_Predict
...
def plot_label_hist(target, savepath, vocab):
    import matplotlib.pyplot as plt 

    if isinstance(target, list):
        target = np.concatenate(target, axis=0)
    
    pickle.dump(target, open(savepath.replace('png', 'pkl'), 'wb'))
    
    target[target > 0.5] = 1
    target[target <= 0.5] = 0
    target_idx = np.where(target == 1)[1]
    
    fig, ax = plt.subplots(figsize=(10,20))
    bins = np.arange(len(target[0])+1) - 0.5 # 0-145 -> -0.5,0.5,1.5...144.5,145.5
    assert len(bins) == len(vocab.idx2word) + 1
    n, bins, patches = ax.hist(target_idx, bins=bins, edgecolor='black', orientation="horizontal")
    ax.set_title('Labels histogram')
    ax.set_ylabel("Medication", fontsize=5.5)
    ax.set_yticks((bins + 0.5)[:-1], labels=[vocab.idx2word[i] for i in range(len(target[0]))])
    ax.set_ylim(-0.5, )
...
def main():
    ...
    # Required parameters
    parser.add_argument("--model_name", default='Model_checking', type=str, required=False,
                        help="model name")
    ...
    parser.add_argument("--pretrain_dir", default='../saved/Model_checking-pretraining', type=str, required=False,
    ...
    if args.use_pretrain:
        ...
        model = GBERTNotesCheck_Predict.from_pretrained(
        ...
        config.graph = args.graph
        model = GBERTNotesCheck_Predict(config, tokenizer,
                    mlp_input_dim=768, # TODO: get this from embedding info prior to training
                    mlp_hidden_dims=[256, 64])
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for _, batch in enumerate(prog_iter):
                ...
                input_embs = torch.zeros_like(input_embs) # TODO 
                ...
                rx_y_preds.append(t2n(rx_logits))

            plot_label_hist(rx_y_trues, '../data/hist_true_label_train.png', tokenizer.rx_voc_multi)
            plot_label_hist(rx_y_preds, '../data/hist_pred_label_train.png', tokenizer.rx_voc_multi)
            ...
            if args.do_eval:
                ...
                print('')
                plot_label_hist(rx_y_trues, '../data/hist_true_label_val.png', tokenizer.rx_voc_multi)
                plot_label_hist(rx_y_preds, '../data/hist_pred_label_val.png', tokenizer.rx_voc_multi)
                ...
                rx_acc_container = metric_report(np.concatenate(rx_y_preds, axis=0), np.concatenate(rx_y_trues, axis=0),
                                                 args.therhold)
                # rx_y_preds = np.concatenate(rx_y_preds, axis=0)
                # rx_y_preds[rx_y_preds > args.therhold] = 1
                # rx_y_preds[rx_y_preds <= args.therhold] = 0
                # import pdb;pdb.set_trace()
    ...
    if args.do_test:
        ...
        def test(task=0):
            ...
            print('')
            plot_label_hist(y_trues, '../data/hist_true_label_test.png', tokenizer.rx_voc_multi)
            plot_label_hist(y_preds, '../data/hist_pred_label_test.png', tokenizer.rx_voc_multi)
